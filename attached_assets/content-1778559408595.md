[Skip to content](https://rollupjs.org/configuration-options/#VPContent)
...
Note that [source phase imports](https://rollupjs.org/es-module-syntax/#source-phase-import) (`import source x from 'y'`) are required to be external. Rollup will raise an error if a source phase import resolves to a module that is not external.
...
### jsx [​](https://rollupjs.org/configuration-options/\#jsx)
...
#### jsx.fragment [​](https://rollupjs.org/configuration-options/\#jsx-fragment)
...
### logLevel [​](https://rollupjs.org/configuration-options/\#loglevel)
...
### maxParallelFileOps [​](https://rollupjs.org/configuration-options/\#maxparallelfileops)
...
See also [`output.intro/output.outro`](https://rollupjs.org/configuration-options/#output-intro-output-outro).
...
- `isImplicitEntry`: `true` if this chunk was emitted with [`implicitlyLoadedAfterOneOf`](https://rollupjs.org/plugin-development/#this-emitfile) set, indicating it will only be loaded as an entry point if at least one of the specified modules have already been loaded.
...
function _interopNamespaceCompat
(e
) {
  	if (e
&& typeof e
...
var external__namespace
= /*#__PURE__*/ _interopNamespaceCompat
(external
);
...
()
  	.then
(function () {
  		return /*#__PURE__*/ _interopNamespaceCompat
(require
('external2'));
  	})
  	.then
...
#### output.amd.define [​](https://rollupjs.org/configuration-options/\#output-amd-define)
...
### output.noConflict [​](https://rollupjs.org/configuration-options/\#output-noconflict)
...
### output.sanitizeFileName [​](https://rollupjs.org/configuration-options/\#output-sanitizefilename)
...
#### treeshake.unknownGlobalSideEffects [​](https://rollupjs.org/configuration-options/\#treeshake-unknownglobalsideeffects)
...
### watch.allowInputInsideOutputPath [​](https://rollupjs.org/configuration-options/\#watch-allowinputinsideoutputpath)
...
### watch.buildDelay [​](https://rollupjs.org/configuration-options/\#watch-builddelay)
...
### watch.chokidar [​](https://rollupjs.org/configuration-options/\#watch-chokidar)
...
### watch.clearScreen [​](https://rollupjs.org/configuration-options/\#watch-clearscreen)
...
### watch.exclude [​](https://rollupjs.org/configuration-options/\#watch-exclude)
...
### watch.include [​](https://rollupjs.org/configuration-options/\#watch-include)
...
### watch.skipWrite [​](https://rollupjs.org/configuration-options/\#watch-skipwrite)
...
### watch.onInvalidate [​](https://rollupjs.org/configuration-options/\#watch-oninvalidate)

| Type: | `(id: string) => void` |

An optional function that will be called immediately every time a module changes that is part of the build. It receives the id of the changed module as argument. This is different from the [`watchChange`](https://rollupjs.org/plugin-development/#watchchange) plugin hook, which is only called once the running build has finished. This may for instance be used to prevent additional steps from being performed if we know another build will be started anyway once the current build finished. This callback may be called multiple times per build as it tracks every change.

## Deprecated options [​](https://rollupjs.org/configuration-options/\#deprecated-options)

☢️ These options have been deprecated and may be removed in a future Rollup version.

### output.externalImportAssertions [​](https://rollupjs.org/configuration-options/\#output-externalimportassertions)

_Use the [`output.externalImportAttributes`](https://rollupjs.org/configuration-options/#output-externalimportattributes) option instead._
...
### output.onlyExplicitManualChunks [​](https://rollupjs.org/configuration-options/\#output-onlyexplicitmanualchunks)

| Type: | `boolean` |

If set to true, using the [output.manualChunks](https://rollupjs.org/configuration-options/#output-manualchunks) function form won't merge dependencies into the output chunk.
